...
## 11. Changelog: fixes made same-day (2026-09-24), continued session -- `keyword`/`name`, `clojure.walk`, and three more foundational compiler bugs

A second pass the same day, adding `keyword`/`name` (to unblock `clojure.walk`) and then `clojure.walk` itself, surfaced three more real, previously-unknown bugs -- all in core compiler machinery, none specific to the two features being added. Found the same way as Section 10: by running real code and comparing actual output to expected, then root-causing rather than working around. All four items below are fixed, verified individually, and confirmed against both full test suites (129/129, 125/125) after each fix.

- **`keyword`/`name`**: were completely missing (`Undefined function`), despite the underlying runtime primitives (`clorus_keyword`, interning correctly; `clorus_keyword_name`) already existing -- the same "implemented but unreachable" pattern as `char-at`/`index-of` in Section 10. Added a new `clorus_name` (broader than the existing keyword-only helper: handles Keyword, Symbol, and String, matching real Clojure's `(name x)`), wired both into the compiler's core-call dispatch and the function allowlist.
- **Closures with a non-trivial body silently dropped captures**: `collect_free_vars`, which determines what a nested `fn`/closure must capture from its enclosing scope, matched only 10 of the AST's ~45 expression variants and silently no-op'd on everything else via a wildcard arm -- the same silent-fallback bug class as the `ValueTag` audit in Section 10, just in the AST layer. In particular `(do ...)` bodies were unhandled, so any closure doing more than one thing (an extremely common shape) failed to compile the moment it referenced anything from its enclosing scope, with a confusing "Undefined variable" error pointing at the *use*, not the real cause. Fixed by making the match exhaustive (removing the wildcard, letting the compiler enumerate every remaining variant) and correctly classifying each one, including giving `SyntaxQuote` its own quote-aware traversal so only `~unquote`/`~@unquote-splicing` forms inside it are treated as live variable references, not every symbol that happens to appear in quoted template code.
- **JIT-only miscompilation of redefined/collision-named functions**: two call-compilation sites determined a function's calling convention (fixed-arg count, has-rest-param) by reading `function.get_name()` off the resolved LLVM value, instead of the lookup key that had actually found it. If that name collided with an LLVM global already defined earlier in the same module -- e.g. simply *using* the stock `partial` from `stdlib/clorus/core.clr` (which is itself compiled earlier in the same module) -- LLVM silently renames the colliding global rather than erroring, the two names diverge, and the signature lookup misses. At a direct call site this meant no fallback at all: extra arguments were passed straight through into the callee's rest-parameter slot unwrapped, instead of being collected into a vector first. This is why `(map (partial + 10) [1 2 3])` returned garbage under `clorus run` (JIT, the default) while identical logic under a non-colliding name, and the exact same code under `--legacy-run` or the REPL, worked fine. A related, separate bug in the dedicated compile path for `ns/fn`-qualified calls (used by any `:as`-aliased require) did no arity-variant selection at all, unconditionally calling through a multi-arity function's *last-declared* arity regardless of how many arguments the call site supplied -- silently reading garbage for any parameter beyond what was actually passed rather than erroring. Both fixed by threading the actual resolved lookup key through instead of re-deriving one from the (possibly-renamed) LLVM symbol, and by giving the qualified-call path the same arity-variant selection and rest-vector shaping the unqualified path already had.
- **JIT compiled the entire stdlib twice, only ever executing one copy**: `clorus run` compiled every stdlib form (core.clr + transducers.clr) twice -- once so `load_and_compile_modules` could resolve stdlib references while compiling required module files, and again as part of the main expression-execution loop, which is the only one of the two whose wrapper functions actually get called. The first pass's wrapper bodies -- where every `defn`'s side-effecting "Var-backed global" gets its initial value (the mechanism used whenever a function is referenced as a bare *value* rather than called directly, e.g. `conj` in stdlib's own `(reduce conj to from)` inside `into`) -- were dead code, compiled but never invoked. Any required module file, compiled strictly between the two passes, could only ever see the first pass's copy of a stdlib function at its own compile time, since the second pass hadn't run yet -- so any such module calling a stdlib function that itself references another stdlib symbol as a value (like `into`) silently read an uninitialized global instead of the real value, e.g. `(into [] coll)` returned `[]`, discarding `coll` entirely, instead of erroring. Entry-file code was unaffected, since it's compiled as part of the same second pass that does run -- which is why this only reproduced through a required module, and why `--legacy-run` (a real object file/executable, no two-pass structure) never showed it. Fixed by not recompiling the stdlib portion a second time, instead threading the first pass's (and `load_and_compile_modules`'s own) wrapper names directly into the executed sequence. Also incidentally made JIT startup noticeably faster, since stdlib is no longer built twice.
- **`clojure.walk`**: added (`walk`, `postwalk`, `prewalk`, `postwalk-replace`, `prewalk-replace`, `keywordize-keys`, `stringify-keys`), ported from Clojure's own `clojure.walk` minus the JVM-only `IMapEntry`/`IRecord` special cases (Clorus maps are already seqable as `[k v]` vector pairs, so the generic collection branch handles them without extra help).

**Process note, continued:** three of the four bugs above are in shared compiler infrastructure (free-variable capture, call-signature resolution, JIT module compilation) rather than in any one stdlib function, and none were specific to `keyword`/`name`/`clojure.walk` -- they were simply the first things in this session's work to exercise those particular paths (a closure with a `do` body; calling a function whose name happens to collide with an already-compiled one; calling a stdlib function-that-references-another-function-by-value from a required module file). This reinforces Section 10's point: passing tests is not evidence a path is correct, only that it's been exercised. The root-causing discipline that found the JIT double-compilation bug specifically was to notice that a *fresh, uniquely-named* function with identical logic worked while the real one didn't, then narrow the difference systematically (unqualified vs. qualified call, direct call vs. call-through-a-required-module, JIT vs. `--legacy-run`) rather than guessing at the fix.
