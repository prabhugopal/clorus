...
- Version prerelease/build segments are also trim-normalized before comparison to avoid whitespace-sensitive ordering drift.
...
- Empty package ids are rejected even when mixed with non-empty candidates (no ambiguous-resolution fallback).
...
- `resolve.root` is trim-normalized before node lookup; whitespace-padded values are accepted.
- If `resolve.root` is present but empty after trimming, resolver errors explicitly (metadata inconsistency).
- If `resolve.root` matches multiple resolve nodes, resolver now errors explicitly (metadata inconsistency).
...
- Registry metadata-resolution failures now normalize stderr (including whitespace and empty-stderr handling) and add explicit network-unavailable guidance for crates.io DNS/download failures (with case-insensitive host/download matching, and without adding that hint for non-network errors).
...
- local path dependency + explicit `.clri` interface path includes mixed `:string` + `:str` mappings with distinct wrapper call-shape assertions (`String` vs `&str`)
- explicit `.clri` + `:rust` override E2E coverage includes `:str` mapping to `&str` and override call-site assertions
...
- version dependency `interface = false` E2E path confirms co-located interface files are ignored (registry auto-parse path remains authoritative)
...
- dependency-context idempotence is regression-covered for whitespace-prefixed registry no-FFI-compatible diagnostics (no duplicate dependency labels)
- dependency-context idempotence is regression-covered for whitespace-prefixed registry metadata-resolution diagnostics (no duplicate dependency labels)
- dependency-context wrapping is regression-covered for registry metadata-resolution diagnostics labeled for a different dependency (mismatched labels are re-wrapped with the current dependency context)
