...
- Registry metadata-resolution failures now normalize stderr (including whitespace and empty-stderr handling) and add explicit network-unavailable guidance for crates.io DNS/download failures (without adding that hint for non-network errors).
