...
Resolved package id: {}. \
Re-run cargo metadata and check dependency graph consistency.",
                    dep_name, resolved_version, resolved_pkg_id
                ));
            }
            Ok(PathBuf::from(src))
        };
...
resolve.root '{}' matched multiple resolve nodes. Re-run cargo metadata and check dependency graph consistency.",
            ...
            let mut root_matches = root_node
                .deps
                .iter()
                .find(|d| d.name.trim() == dep_name)
                .map(|_| {
                    root_node
                        .deps
                        .iter()
                        .filter(|d| d.name.trim() == dep_name)
...
Re-run cargo metadata and check dependency graph consistency.",
            ...
            if packages.iter().any(|pkg| pkg.is_empty()) {
                return Err(format!(
...
Root '{}' resolves to package ids: [{}]. Pin the dependency explicitly in Cargo/Clorus.toml or use a path bridge crate.",
            ...
            let mut pkg_ids = resolve
                ...
                .map(|d| d.pkg.trim().to_string())
                .collect::<Vec<String>>();
            if pkg_ids.iter().any(|pkg| pkg.is_empty()) {
                return Err(format!(
                    "Registry dependency '{}' resolve graph contained an empty package id for dependency edge '{}'. \
Re-run cargo metadata and check dependency graph consistency.",
                    dep_name, dep_name
...
mod tests {
    ...
    #[test]
    fn resolve_registry_lib_src_supports_whitespace_wrapped_lib_target_kind() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_normalizes_target_kinds_in_root_no_lib_diagnostic() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
                ...
                CargoPackage {
                    ...
                    targets: vec![
                        CargoTarget {
                            kind: vec![" test ".to_string(), " bin ".to_string()],
                            src_path: "/tmp/registry/v0_2_0/src/test.rs".to_string(),
                        },
                        CargoTarget {
                            kind: vec![" bin ".to_string()],
                            src_path: "/tmp/registry/v0_2_0/src/main.rs".to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_uses_none_target_kinds_in_root_no_lib_diagnostic() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_trims_version_in_root_no_lib_diagnostic() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_normalizes_pkg_id_in_empty_lib_src_path_diagnostic() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_trims_version_in_empty_lib_src_path_diagnostic() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_supports_root_missing_unique_edge_with_whitespace_dep_name() {
        let metadata = CargoMetadata {
            packages: vec![CargoPackage {
                id: "registry+https://github.com/rust-lang/crates.io-index#demo-math@0.2.0"
                    .to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_errors_when_root_missing_edges_mix_empty_and_non_empty_package_ids() {
        let metadata = CargoMetadata {
            packages: vec![],
            resolve: Some(CargoResolve {
                root: None,
        ...
        let err = RustFfiProcessor::resolve_registry_lib_src(&metadata, "demo-math")
            .expect_err("should fail when root-missing edges contain any empty package id");
        assert!(err.contains("contained an empty package id"));
        assert!(err.contains("dependency edge 'demo-math'"));
    }
    ...
    #[test]
    fn resolve_registry_lib_src_errors_when_root_edge_mix_empty_and_non_empty_package_ids() {
        let metadata = CargoMetadata {
            packages: vec![CargoPackage {
                id: "path+file:///tmp/wrapper#0.1.0".to_string(),
                name: "demo_wrapper".to_string(),
        ...
        let err = RustFfiProcessor::resolve_registry_lib_src(&metadata, "demo-math")
            .expect_err("should fail when root edge package ids include an empty value");
        assert!(err.contains("contained an empty package id"));
        assert!(err.contains("dependency edge 'demo-math'"));
    }
    ...
    #[test]
    fn resolve_registry_lib_src_supports_root_edge_with_whitespace_dep_name() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_normalizes_version_in_non_registry_diagnostic() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
    ...
    #[test]
    fn resolve_registry_lib_src_errors_when_root_resolution_points_to_unknown_source() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
        ...
        let err = RustFfiProcessor::resolve_registry_lib_src(&metadata, "demo-math")
            .expect_err("should fail when root points to non-registry source with unknown source field");
        assert!(err.contains("resolved to non-registry package source '<unknown>'"));
        assert!(err.contains("pkg id path+file:///tmp/local/demo-math#0.2.0"));
    }

    #[test]
    fn resolve_registry_lib_src_errors_when_root_resolution_points_to_whitespace_source() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
        ...
        let err = RustFfiProcessor::resolve_registry_lib_src(&metadata, "demo-math")
            .expect_err("should fail when root points to non-registry source with whitespace source");
        assert!(err.contains("resolved to non-registry package source '<unknown>'"));
        assert!(err.contains("pkg id path+file:///tmp/local/demo-math#0.2.0"));
    }

    #[test]
    fn resolve_registry_lib_src_errors_when_root_missing_unique_edge_points_to_non_registry_source() {
        let metadata = CargoMetadata {
            packages: vec![CargoPackage {
                id: "path+file:///tmp/local/demo-math#0.2.0".to_string(),
                name: "demo-math".to_string(),
        ...
        let err = RustFfiProcessor::resolve_registry_lib_src(&metadata, "demo-math")
            .expect_err("should fail when unique root-missing edge points to non-registry source");
        assert!(err.contains("resolved to non-registry package source 'path+file:///tmp/local/demo-math'"));
        assert!(err.contains("pkg id path+file:///tmp/local/demo-math#0.2.0"));
    }
    ...
    #[test]
    fn resolve_registry_lib_src_prioritizes_empty_version_over_root_no_lib_diagnostic() {
        let metadata = CargoMetadata {
            packages: vec![
                CargoPackage {
                    id: "path+file:///tmp/wrapper#0.1.0".to_string(),
        ...
        let err = RustFfiProcessor::resolve_registry_lib_src(&metadata, "demo-math")
            .expect_err("should fail on empty version before no-lib diagnostics");
        assert!(err.contains("empty version"));
        assert!(!err.contains("has no lib target"));
    }
...
"#,
    ...
    #[test]
    fn process_dependencies_e2e_explicit_legacy_interface_supports_str_keyword() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "clorus_rustffi_e2e_explicit_legacy_str_{}",
            unique
        ));
        let dep_dir = root.join("legacy-str-lib");
        let iface_dir = root.join("interfaces");
        std::fs::create_dir_all(dep_dir.join("src")).expect("create dep src");
        std::fs::create_dir_all(&iface_dir).expect("create interfaces dir");
...
"#,
        ...
        std::fs::write(
            iface_dir.join("legacy-str-lib.clorus-ffi"),
            r#"(interface legacy-str-lib
...
[rust-dependencies]
legacy-str-lib = { path = "legacy-str-lib", interface = "interfaces/legacy-str-lib.clorus-ffi" }
"#,
        )
        .expect("parse manifest");
...
"#,
    ...
    fn process_dependencies_e2e_explicit_legacy_str_with_rust_symbol_override() {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "clorus_rustffi_e2e_explicit_legacy_str_override_{}",
            unique
        ));
        let dep_dir = root.join("legacy-str-override-lib");
        let iface_dir = root.join("interfaces");
        std::fs::create_dir_all(dep_dir.join("src")).expect("create dep src");
        std::fs::create_dir_all(&iface_dir).expect("create interfaces dir");
...
"#,
        ...
        std::fs::write(
            iface_dir.join("legacy-str-override-lib.clorus-ffi"),
            r#"(interface legacy-str-override-lib
...
[rust-dependencies]
legacy-str-override-lib = { path = "legacy-str-override-lib", interface = "interfaces/legacy-str-override-lib.clorus-ffi" }
"#,
        )
        .expect("parse manifest");
        ...
        assert_eq!(processed.libraries.len(), 1);
        let lib = &processed.libraries[0];
        assert_eq!(lib.name, "legacy_str_override_lib_ffi");
        let f = lib
            .functions
            .iter()
            .find(|f| f.name == "greet")
        ...
        let wrapper_src = root
            .join("target")
            .join("rust-ffi")
            .join("legacy_str_override_lib_ffi")
            .join("src")
            .join("lib.rs");
        let generated = std::fs::read_to_string(&wrapper_src).expect("read wrapper source");
        assert!(generated.contains("let s_rust_owned = unsafe { CStr::from_ptr(s as *const c_char).to_string_lossy().to_string() };"));
        ...
        assert!(generated.contains("fn clorus_legacy_str_override_lib__greet("));
...
"#,
    ...
    #[test]
    fn process_dependencies_e2e_explicit_clri_supports_string_and_str_keywords_distinctly() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "clorus_rustffi_e2e_explicit_clri_string_and_str_{}",
            unique
        ));
        let dep_dir = root.join("explicit-clri-string-str-lib");
        let iface_dir = root.join("interfaces");
        std::fs::create_dir_all(dep_dir.join("src")).expect("create dep src");
        std::fs::create_dir_all(&iface_dir).expect("create interfaces dir");
        ...
        std::fs::write(
            dep_dir.join("Cargo.toml"),
            r#"[package]
name = "explicit-clri-string-str-lib"
version = "0.1.0"
edition = "2021"
"#,
        )
...
"#,
        ...
        std::fs::write(
            iface_dir.join("explicit-clri-string-str-lib.clri"),
            r#"(interface explicit-clri-string-str-lib
  (fn echo-string [s :string] :string :rust "echo_string")
  (fn echo-str [s :str] :str :rust "echo_str")
)"#,
        )
...
[rust-dependencies]
explicit-clri-string-str-lib = { path = "explicit-clri-string-str-lib", interface = "interfaces/explicit-clri-string-str-lib.clri" }
"#,
        )
        .expect("parse manifest");
        ...
        let wrapper_src = root
            .join("target")
            .join("rust-ffi")
            .join("explicit_clri_string_str_lib_ffi")
            .join("src")
            .join("lib.rs");
        let generated = std::fs::read_to_string(&wrapper_src).expect("read wrapper source");
        assert!(
...
"#,
    ...
    #[test]
    fn process_dependencies_e2e_explicit_trimmed_clri_interface_with_rust_symbol_override() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "clorus_rustffi_e2e_explicit_trimmed_clri_override_{}",
            unique
        ));
        let dep_dir = root.join("trimmed-clri-override-lib");
        let iface_dir = root.join("interfaces");
        std::fs::create_dir_all(dep_dir.join("src")).expect("create dep src");
        std::fs::create_dir_all(&iface_dir).expect("create interfaces dir");
        ...
        std::fs::write(
            dep_dir.join("Cargo.toml"),
            r#"[package]
name = "trimmed-clri-override-lib"
version = "0.1.0"
edition = "2021"
"#,
        )
...
"#,
        ...
        std::fs::write(
            iface_dir.join("trimmed-clri-override-lib.clri"),
            r#"(interface trimmed-clri-override-lib
  (fn answer [] :u64 :rust "Math::answer")
)"#,
        )
        .expect("write clri interface");
...
[rust-dependencies]
trimmed-clri-override-lib = { path = "trimmed-clri-override-lib", interface = "  interfaces/trimmed-clri-override-lib.clri  " }
"#,
        )
        .expect("parse manifest");
        ...
        assert_eq!(processed.libraries.len(), 1);
        let lib = &processed.libraries[0];
        assert_eq!(lib.name, "trimmed_clri_override_lib_ffi");
        let answer_fn = lib
            .functions
            .iter()
            .find(|f| f.name == "answer")
        ...
        let wrapper_src = root
            .join("target")
            .join("rust-ffi")
            .join("trimmed_clri_override_lib_ffi")
            .join("src")
            .join("lib.rs");
        let generated = std::fs::read_to_string(&wrapper_src).expect("read wrapper source");
        assert!(
            generated.contains("let result = Math::answer();"),
            "expected trimmed clri interface :rust override callsite in generated wrapper:\n{}",
            generated
        );
...
"#,
    ...
    #[test]
    fn process_dependencies_e2e_rejects_unsupported_legacy_interface_pointer_type() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "clorus_rustffi_e2e_reject_unsupported_legacy_pointer_type_{}",
            unique
        ));
        let dep_dir = root.join("unsupported-legacy-pointer-lib");
        let iface_dir = root.join("interfaces");
        std::fs::create_dir_all(dep_dir.join("src")).expect("create dep src");
        std::fs::create_dir_all(&iface_dir).expect("create interfaces dir");
        ...
        std::fs::write(
            dep_dir.join("Cargo.toml"),
            r#"[package]
name = "unsupported-legacy-pointer-lib"
version = "0.1.0"
edition = "2021"
"#,
        )
...
"#,
        ...
        std::fs::write(
            iface_dir.join("unsupported-legacy-pointer-lib.clorus-ffi"),
            r#"(interface unsupported-legacy-pointer-lib
  (fn ptr-roundtrip [p :ptr-i32] :ptr-i32)
)"#,
        )
        .expect("write legacy interface");
...
[rust-dependencies]
unsupported-legacy-pointer-lib = { path = "unsupported-legacy-pointer-lib", interface = "interfaces/unsupported-legacy-pointer-lib.clorus-ffi" }
"#,
        )
        .expect("parse manifest");

        let err = with_cwd(&root, || match RustFfiProcessor::process_dependencies(&manifest, false) {
            Ok(_) => panic!("expected unsupported legacy interface pointer-type rejection"),
            Err(e) => e,
        });
        assert!(err.contains("Rust dependency 'unsupported-legacy-pointer-lib':"));
        assert!(
            err.contains("unsupported param type"),
            "expected unsupported param-type rejection, got: {}",
            err
        ...
        assert!(
            err.contains("unsupported-legacy-pointer-lib.clorus-ffi"),
    ...
    #[test]
    fn process_dependencies_e2e_rejects_unsupported_legacy_interface_return_type() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!(
            "clorus_rustffi_e2e_reject_unsupported_legacy_return_type_{}",
            unique
        ));
        let dep_dir = root.join("unsupported-legacy-return-lib");
        let iface_dir = root.join("interfaces");
        std::fs::create_dir_all(dep_dir.join("src")).expect("create dep src");
        std::fs::create_dir_all(&iface_dir).expect("create interfaces dir");
        ...
        std::fs::write(
            dep_dir.join("Cargo.toml"),
            r#"[package]
name = "unsupported-legacy-return-lib"
version = "0.1.0"
edition = "2021"
"#,
        )
...
"#,
        ...
        std::fs::write(
            iface_dir.join("unsupported-legacy-return-lib.clorus-ffi"),
            r#"(interface unsupported-legacy-return-lib
  (fn make-bytes [] :Vec<u8> :rust "make_bytes")
)"#,
        )
        .expect("write legacy interface");
...
[rust-dependencies]
unsupported-legacy-return-lib = { path = "unsupported-legacy-return-lib", interface = "interfaces/unsupported-legacy-return-lib.clorus-ffi" }
"#,
        )
        .expect("parse manifest");

        let err = with_cwd(&root, || match RustFfiProcessor::process_dependencies(&manifest, false) {
            Ok(_) => panic!("expected unsupported legacy interface return-type rejection"),
            Err(e) => e,
        });
        assert!(err.contains("Rust dependency 'unsupported-legacy-return-lib':"));
        assert!(
            err.contains("unsupported return type 'Vec<u8>'"),
            "expected unsupported return-type rejection, got: {}",
            err
        );
        assert!(
            err.contains("unsupported-legacy-return-lib.clorus-ffi"),
            "expected legacy interface path context, got: {}",
            err
        );
        assert!(
