...
impl Parser {
    ...
    pub fn parse_expr(&mut self) -> Result<Expr, String> {
        match self.current_token() {
            ...
            Token::LParen => self.parse_list_or_special(),
            ...
            Token::RParen | Token::RBracket | Token::RBrace => {
                Err(format!("Unexpected closing delimiter: {:?}", self.current_token()))
            }
    ...
    fn parse_multi_arity_defn(&mut self, name: String) -> Result<Expr, String> {
        ...
        while self.current_token() == &Token::LParen {
            ...
            self.expect(Token::RParen)?; // consume ) closing arity clause
        ...
        self.expect(Token::RParen)?; // consume ) closing defn
    ...
    fn parse_defprotocol(&mut self) -> Result<Expr, String> {
        ...
        while self.current_token() != &Token::RParen {
            ...
            // Optional docstring
            let docstring = if let Token::String(s) = self.current_token() {
                let doc = s.clone();
                self.advance();
                Some(doc)
            } else {
            ...
            self.expect(Token::RParen)?; // consume ) for method
        ...
        self.expect(Token::RParen)?; // consume ) for defprotocol
    ...
    fn parse_extend_type(&mut self) -> Result<Expr, String> {
        ...
        self.expect(Token::RParen)?; // consume ) for extend-type
    ...
    fn parse_multi_arity_fn(&mut self) -> Result<Expr, String> {
        ...
        self.expect(Token::RParen)?; // consume ) closing fn
    ...
    fn parse_pattern(&mut self) -> Result<crate::ast::Pattern, String> {
        ...
        match self.current_token() {
            ...
            Token::LBracket => self.parse_vector_pattern(),
    ...
    fn parse_use(&mut self) -> Result<Expr, String> {
        ...
        let imports = if matches!(self.current_token(), Token::LBracket) {
            self.advance(); // Skip [
    ...
    fn parse_try(&mut self) -> Result<Expr, String> {
        ...
        while self.current_token() != &Token::RParen {
            ...
            match self.current_token() {
                Token::Symbol(s) if s == "catch" => {
                    self.advance(); // consume 'catch'

                    // Parse exception type (optional - if not present, catch all)
                    let exception_type = if matches!(self.current_token(), Token::Symbol(_)) {
                        let next = self.current_token().clone();
                        if let Token::Symbol(type_name) = next {
                            self.advance();
                            Some(type_name)
                        } else {
                            None
                    ...
                    self.expect(Token::RParen)?; // close catch clause
                ...
                Token::Symbol(s) if s == "finally" => {
                    self.advance(); // consume 'finally'

                    // Parse finally expression
                    let finally_expr = self.parse_expr()?;

                    self.expect(Token::RParen)?; // close finally clause
        ...
        self.expect(Token::RParen)?; // close try
    ...
    fn parse_require_spec(&mut self) -> Result<crate::ast::RequireSpec, String> {
        ...
        while !matches!(self.current_token(), Token::RBracket) {
            match self.current_token() {
                Token::Keyword(kw) if kw == "as" => {
                    self.advance(); // Skip :as
                    match self.current_token() {
                        Token::Symbol(alias_name) => {
                ...
                Token::Keyword(kw) if kw == "refer" => {
                    self.advance(); // Skip :refer

                    // Check for :all
                    if matches!(self.current_token(), Token::Keyword(kw) if kw == "all") {
                        refer_all = true;
                        self.advance();
