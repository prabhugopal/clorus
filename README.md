...
- **Concurrency is real but not `core.async`-grade yet.** Atoms, refs/STM, agents, channels, `go`, and `alts!!` all work, and STM (`dosync`/`alter`/`commute`) conflict detection under real concurrent contention was verified with a multi-threaded stress test. `go` blocks still run on a fixed-size worker pool (sized to CPU count) rather than as lightweight suspended state machines — a program with more concurrently-blocked `go` blocks than CPU cores can exhaust the pool — and the STM retry loop has no backoff yet, so heavy contention means a CPU-spin rather than a crash.
